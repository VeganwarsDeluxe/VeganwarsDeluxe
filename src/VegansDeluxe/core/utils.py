...
def per_cubes(cubes: int, accuracy: int, energy=0, additional_accuracies=0, critical=False) -> int:
    total_accuracy = (accuracy + additional_accuracies + energy)
    result = sum(1 for _ in range(cubes) if random.randint(1, 10) <= total_accuracy)
    if critical and total_accuracy > 10:
        result = int(math.floor(result * total_accuracy / 10))
    return result
